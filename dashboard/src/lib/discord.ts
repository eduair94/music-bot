...
/**
 * Check if bot is in a specific guild (client-safe version)
 * Uses Discord API with caching - for server-side Redis version use discord-server.ts
 */
export async function isBotInGuild(guildId: string): Promise<boolean> {
  // Use Discord API with caching
 ...
 * Client-safe version - for Redis-enhanced version use discord-server.ts
 */
