...
export default {
  data: new SlashCommandBuilder()
    .setName("tremolo")
    .setDescription("Apply tremolo audio effect")
    .addBooleanOption((option) =>
      option.setName("enabled").setDescription("Enable or disable tremolo effect").setRequired(false)
    ),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const isCurrentlyEnabled = currentFilters.includes("tremolo");
    ...
    await logAction(interaction.guild!, interaction.user, "setting_change", `Tremolo ${shouldEnable ? "On" : "Off"}`);

    return interaction.editReply({
      content: `<@${interaction.user.id}> 〰️ Tremolo effect is now ${shouldEnable ? "**enabled**" : "**disabled**"}`
    }).catch(console.error);
  }
};
