...
export default {
  data: new SlashCommandBuilder()
    .setName("vibrato")
    .setDescription("Apply vibrato audio effect")
    .addBooleanOption((option) =>
      option.setName("enabled").setDescription("Enable or disable vibrato effect").setRequired(false)
    ),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const isCurrentlyEnabled = currentFilters.includes("vibrato");
    ...
    await logAction(interaction.guild!, interaction.user, "setting_change", `Vibrato ${shouldEnable ? "On" : "Off"}`);

    return interaction.editReply({
      content: `<@${interaction.user.id}> 📳 Vibrato effect is now ${shouldEnable ? "**enabled**" : "**disabled**"}`
    }).catch(console.error);
  }
};
