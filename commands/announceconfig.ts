    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === "view") {
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: "Text Announcements", value: [
                        `**Enabled:** ${extSettings.textAnnounce !== false ? "‚úÖ" : "‚ùå"}`,
                        `**Auto-Delete:** ${extSettings.textAnnounceAutoDelete ? "‚úÖ" : "‚ùå"}`,
                        `**Extended:** ${extSettings.textAnnounceExtended ? "‚úÖ" : "‚ùå"}`
                    ].join("\n"), inline: true },
                    { name: "Stage Announcements", value: [
                        `**Enabled:** ${extSettings.stageAnnounce ? "‚úÖ" : "‚ùå"}`,
                        `**Template:** ${extSettings.stageTemplate || "Default"}`
                    ].join("\n"), inline: true },
                    { name: "Voice Status", value: [
                        `**Enabled:** ${extSettings.voiceStatusAnnounce ? "‚úÖ" : "‚ùå"}`,
            ...
            await interaction.reply({ embeds: [new EmbedBuilder()
                .setTitle("Ì≥¢ Announcement Settings Reset")
                .setColor(0x2ecc71)
            ...
            if (sub === "toggle") {
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle(enabled ? "Ì≥¢ Text Announcements Enabled" : "Ì≥¢ Text Announcements Disabled")
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle(enabled ? "Ì≥ã Extended Announcements Enabled" : "Ì≥ã Extended Announcements Disabled")
                    .setColor(0x2ecc71)
                    .setDescription(enabled 
            ...
            if (sub === "toggle") {
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle(enabled ? "Ìæ≠ Stage Announcements Enabled" : "Ìæ≠ Stage Announcements Disabled")
                    .setColor(enabled ? 0x2ecc71 : 0xe74c3c)
                    .setDescription(enabled 
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle("Ìæ≠ Stage Template Updated")
                    .setColor(0x2ecc71)
            ...
            if (sub === "toggle") {
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle(enabled ? "Ì¥ä Voice Status Enabled" : "ÔøΩÔøΩ Voice Status Disabled")
                    .setColor(enabled ? 0x2ecc71 : 0xe74c3c)
                    .setDescription(enabled 
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle("Ì¥ä Voice Status Template Updated")
                    .setColor(0x2ecc71)
                ...
                await interaction.reply({ embeds: [new EmbedBuilder()
                    .setTitle("Ì¥ä Default Voice Status Set")
                    .setColor(0x2ecc71)
