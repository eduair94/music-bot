...
export default {
  data: new SlashCommandBuilder()
    .setName("distortion")
    .setDescription("Apply distortion audio effect")
    .addBooleanOption((option) =>
      option.setName("enabled").setDescription("Enable or disable distortion effect").setRequired(false)
    ),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const isCurrentlyEnabled = currentFilters.includes("flanger");
    ...
    await logAction(interaction.guild!, interaction.user, "setting_change", `Distortion ${shouldEnable ? "On" : "Off"}`);

    return interaction.editReply({
      content: `<@${interaction.user.id}> ⚡ Distortion effect is now ${shouldEnable ? "**enabled**" : "**disabled**"}`
    }).catch(console.error);
  }
};
