...
export default {
  data: new SlashCommandBuilder()
    .setName("backward")
    .setDescription("Wind the track backwards")
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    await logAction(interaction.guild!, interaction.user, "backward", `${seconds}s`);
    
    return interaction.editReply({
      content: `<@${interaction.user.id}> ⏪ rewound **${seconds}s** (now at ${formatTime(newPosition)})`
    }).catch(console.error);
  }
};
