    ...
    try {
      ...
      if (!result) {
        return interaction.editReply({
          content: `‚ÑπÔ∏è ${targetUser.username} already has default settings (no custom bitrate set).`
        }).catch(console.error);
      }

      return interaction.editReply({
        content: `‚úÖ **Audio settings reset for ${targetUser.username}**\n\n` +
                 `**Quality:** Ì¥â 128kbps (Free/Default)\n` +
      ...
      return interaction.editReply({
        content: `‚ùå Error resetting bitrate: ${error.message || "Unknown error"}`
      }).catch(console.error);
    }
  }
};
