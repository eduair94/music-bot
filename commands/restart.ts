...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!queue || !queue.currentTrack) {
      return interaction.editReply({ content: "❌ There is no track playing." }).catch(console.error);
    }
    ...
    await logAction(interaction.guild!, interaction.user, "restart", currentTrack.title);
    
    return interaction.editReply({
      content: `<@${interaction.user.id}> 🔄 restarted **${currentTrack.title}**`
    }).catch(console.error);
  }
};
