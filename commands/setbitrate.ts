    ...
    try {
      ...
      if (!patron) {
        return interaction.editReply({
          content: "‚ùå Failed to update bitrate settings."
        }).catch(console.error);
      }

      // Build response message
      const qualityLabel = 
        bitrate >= 320 ? "Ì¥ä HQ 320kbps (Premium)" :
        bitrate >= 256 ? "Ì¥ä 256kbps (High)" :
        bitrate >= 192 ? "Ì¥â 192kbps (Mid)" :
        "Ì¥â 128kbps (Free)";
      ...
      return interaction.editReply({
        content: `‚úÖ **Audio settings updated for ${targetUser.username}**\n\n` +
      ...
      return interaction.editReply({
        content: `‚ùå Error updating bitrate: ${error.message || "Unknown error"}`
      }).catch(console.error);
    }
  }
};
