...
export default {
  data: new SlashCommandBuilder()
    .setName("removecurrent")
    .setDescription("Remove the currently playing track and skip to the next one"),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Skip to next track (this effectively removes current from history)
    queue.node.skip();

    await logAction(interaction.guild!, interaction.user, "removecurrent", trackTitle);

    return interaction.editReply({
      content: `🗑️ Removed and skipped **${trackTitle}**`
    }).catch(console.error);
  }
};
