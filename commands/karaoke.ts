...
export default {
  data: new SlashCommandBuilder()
    .setName("karaoke")
    .setDescription("Apply karaoke effect (reduces vocals)")
    .addBooleanOption((option) =>
      option.setName("enabled").setDescription("Enable or disable karaoke effect").setRequired(false)
    ),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const isCurrentlyEnabled = currentFilters.includes("karaoke");
    ...
    await logAction(interaction.guild!, interaction.user, "setting_change", `Karaoke ${shouldEnable ? "On" : "Off"}`);

    return interaction.editReply({
      content: `<@${interaction.user.id}> 🎤 Karaoke effect is now ${shouldEnable ? "**enabled**" : "**disabled**"}`
    }).catch(console.error);
  }
};
