...
export class SpotifyService {
  ...
  public async searchTracks(query: string): Promise<SpotifyTrack[]> {
    try {
      console.log(`[SpotifyService] ��� Searching for: "${query}"`);
      ...
      if (!response.data?.tracks || response.data.tracks.length === 0) {
        console.log(`[SpotifyService] ❌ No results found for: "${query}"`);
        return [];
      }

      const tracks = response.data.tracks.map(result => this.formatTrack(result.data));
      console.log(`[SpotifyService] ✅ Found ${tracks.length} tracks`);
  ...
  public async searchTrack(query: string): Promise<SpotifyTrack | null> {
    ...
    if (cached && cached.expires > Date.now()) {
      console.log(`[SpotifyService] ��� Cache hit for: "${query}"`);
      return cached.data;
    }
  ...
  public async searchLyrics(query: string): Promise<SpotifyLyrics | null> {
    try {
      ...
      if (data?.error || lines.length === 0) {
        console.log(`[SpotifyService] ❌ No lyrics found for: "${query}"`);
        return null;
      }
      ...
      console.error("[SpotifyService] ❌ Lyrics error:", error.message);
      return null;
    }
  }
  ...
  public clearCache(): void {
    this.cache.clear();
    console.log("[SpotifyService] ���️ Cache cleared");
  }
